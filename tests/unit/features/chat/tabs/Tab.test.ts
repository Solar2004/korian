...
jest.mock('@/providers/claude/runtime/ClaudeChatRuntime', () => ({
  KorianService: jest.fn().mockImplementation(() => ({
...
const createMockKorianService = (overrides?: {
...
describe('Tab - Service Initialization', () => {
  ...
  describe('initializeTabService', () => {
    it('should not reinitialize if already initialized', async () => {
      ...
      tab.service = createMockKorianService() as any;
    ...
    it('should create KorianService on first initialization', async () => {
      const options = createMockOptions();
      const tab = createTab(options);
    ...
    it('should create the runtime for the conversation provider', async () => {
      ...
      const mockRuntime = createMockKorianService({ providerId: 'codex' });
    ...
    it('should recreate the runtime when the conversation provider changes', async () => {
      const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime');
      const oldService = createMockKorianService({ providerId: 'claude' });
      const newService = createMockKorianService({ providerId: 'codex' });
    ...
    it('should NOT call ensureReady for blank tabs (lazy start)', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/claude/runtime/ClaudeChatRuntime') as { KorianService: jest.Mock };
      runtimeModule.KorianService.mockImplementationOnce(() => createMockKorianService({ ensureReady: mockEnsureReady }));
    ...
    it('should sync existing conversations with saved external contexts', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/claude/runtime/ClaudeChatRuntime') as { KorianService: jest.Mock };
      runtimeModule.KorianService.mockImplementationOnce(() => createMockKorianService({
    ...
    it('falls back blank Codex draft to Claude when Codex is disabled', () => {
      ...
      const staleService = createMockKorianService({ providerId: 'codex' });
    ...
    it('maps shared permission mode selections onto managed OpenCode modes', async () => {
      const plugin = createMockPlugin({
        settings: {
          ...
          providerConfigs: {
            opencode: {
              availableModes: [
                { id: 'korian-yolo', name: 'YOLO' },
                { id: 'korian-safe', name: 'Safe' },
                { id: 'plan', name: 'Plan' },
              ],
              enabled: true,
              selectedMode: 'korian-yolo',
            },
          },
          savedProviderEffort: {
            claude: 'high',
      ...
      expect(plugin.settings.providerConfigs.opencode.selectedMode).toBe('korian-safe');
...
describe('Tab - Destruction', () => {
  describe('destroyTab', () => {
    ...
    it('should unsubscribe from ready state changes when tab is destroyed', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/claude/runtime/ClaudeChatRuntime') as { KorianService: jest.Mock };
      runtimeModule.KorianService.mockImplementationOnce(() => createMockKorianService({ onReadyStateChange: mockOnReadyStateChange }));
  ...
  describe('setupServiceCallbacks', () => {
    function setupAutoTurnTest() {
      ...
      const addMessage = jest.fn(() => {
        ...
        msgEl.createDiv({ cls: 'korian-message-content' });
        return msgEl;
      });
...
describe('Tab - Service Initialization Error Handling', () => {
  ...
  it('should skip re-initialization if already initialized', async () => {
    ...
    tab.serviceInitialized = true;
    const originalService = createMockKorianService() as any;
...
describe('Tab - First Send Binding', () => {
  it('derives provider from draft model on first send (Claude)', async () => {
    ...
    runtimeModule.KorianService.mockImplementationOnce(() => createMockKorianService({ ensureReady: mockEnsureReady }));
    const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime')
      .mockReturnValue(createMockKorianService() as any);
  ...
  it('derives provider from draft model on first send (Codex)', async () => {
    const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime')
      .mockReturnValue(createMockKorianService({ providerId: 'codex' }) as any);
