...
describe('ModelSelector', () => {
  ...
  it('should create a container with model-selector class', () => {
    const container = parentEl.querySelector('.korian-model-selector');
    expect(container).not.toBeNull();
  });

  it('should display current model label', () => {
    // Default model is 'sonnet' which maps to 'Sonnet'
    const btn = parentEl.querySelector('.korian-model-btn');
    expect(btn).not.toBeNull();
    const label = btn?.querySelector('.korian-model-label');
  ...
  it('should display first model when current model not found', () => {
    ...
    const label = parentEl.querySelector('.korian-model-label');
    expect(label?.textContent).toBe('Haiku');
  });

  it('should render model options in reverse order', () => {
    const dropdown = parentEl.querySelector('.korian-model-dropdown');
  ...
  it('should render group separators when models have group field', () => {
    ...
    const groups = children.filter((c: any) => c.hasClass('korian-model-group'));
  ...
  it('should show 1M variants instead of standard variants when enabled', () => {
    ...
    expect(parentEl.querySelector('.korian-model-label')?.textContent).toBe('Opus 1M');
  });
});
...
describe('ModeSelector', () => {
  ...
  it('should create a container with mode-selector class', () => {
    const container = parentEl.querySelector('.korian-mode-selector');
    expect(container).not.toBeNull();
  });

  it('should display the current mode label', () => {
    const label = parentEl.querySelector('.korian-mode-label');
    expect(label?.textContent).toBe('Build');
  });

  it('should call onModeChange when the toggle is clicked', async () => {
    const toggle = parentEl.querySelector('.korian-toggle-switch');
  ...
  it('should show the active style when the configured active mode is selected', () => {
    ...
    const label = parentEl2.querySelector('.korian-mode-label');
    const toggle = parentEl2.querySelector('.korian-toggle-switch');
    expect(label?.textContent).toBe('Build');
...
describe('ThinkingBudgetSelector', () => {
  ...
  describe('adaptive mode (Claude models)', () => {
    ...
    it('should create a container with thinking-selector class', () => {
      const container = parentEl.querySelector('.korian-thinking-selector');
      expect(container).not.toBeNull();
    });

    it('should show effort selector for Claude models', () => {
      const effort = parentEl.querySelector('.korian-thinking-effort');
      expect(effort).not.toBeNull();
      expect(effort?.style?.display).not.toBe('none');
    });

    it('should hide budget selector for Claude models', () => {
      const budget = parentEl.querySelector('.korian-thinking-budget');
      expect(budget?.style?.display).toBe('none');
    });

    it('should display current effort level for Claude models', () => {
      const current = parentEl.querySelector('.korian-thinking-current');
  ...
  describe('legacy mode (custom models)', () => {
    ...
    it('should render budget options in reverse order', () => {
      const options = parentEl.querySelector('.korian-thinking-options');
...
describe('PermissionToggle', () => {
  ...
  it('should create a container with permission-toggle class', () => {
    const container = parentEl.querySelector('.korian-permission-toggle');
    expect(container).not.toBeNull();
  });

  it('should display Safe label when in normal mode', () => {
    const label = parentEl.querySelector('.korian-permission-label');
  ...
  it('should display YOLO label when in yolo mode', () => {
    ...
    const label = parentEl2.querySelector('.korian-permission-label');
  ...
  it('should hide the control when provider exposes no permission toggle UI', () => {
    ...
    const container = parentEl2.querySelector('.korian-permission-toggle');
    expect(container?.style.display).toBe('none');
  });
...
describe('ServiceTierToggle', () => {
  ...
  it('shows the control when the provider exposes service tier options', () => {
    const container = parentEl.querySelector('.korian-service-tier-toggle');
    expect(container).not.toBeNull();
    expect(container?.hasClass('korian-hidden')).toBe(false);
  });

  it('renders the icon button in the inactive state when fast mode is off', () => {
    const button = parentEl.querySelector('.korian-service-tier-button');
    const icon = parentEl.querySelector('.korian-service-tier-icon');
    const container = parentEl.querySelector('.korian-service-tier-toggle');
  ...
  it('renders the icon button in the active state when fast mode is on', () => {
    ...
    const button = parentEl2.querySelector('.korian-service-tier-button');
    const container = parentEl2.querySelector('.korian-service-tier-toggle');
...
describe('McpServerSelector', () => {
  ...
  it('should create container with mcp-selector class', () => {
    const container = parentEl.querySelector('.korian-mcp-selector');
    expect(container).not.toBeNull();
  });
  ...
  it('should show empty message when all servers are disabled', () => {
    ...
    const empty = parentEl.querySelector('.korian-mcp-selector-empty');
  ...
  it('should show badge when more than 1 server enabled', () => {
    ...
    const badge = parentEl.querySelector('.korian-mcp-selector-badge');
  ...
  it('should add active class to icon when servers are enabled', () => {
    ...
    const icon = parentEl.querySelector('.korian-mcp-selector-icon');
...
describe('ContextUsageMeter', () => {
  ...
  it('should create a container with context-meter class', () => {
    const container = parentEl.querySelector('.korian-context-meter');
    expect(container).not.toBeNull();
  });
  ...
  it('should display percentage', () => {
    meter.update(makeUsage({ contextTokens: 50000, contextWindow: 200000, percentage: 25 }));
    const percent = parentEl.querySelector('.korian-context-meter-percent');
...
describe('McpServerSelector - toggle and badges', () => {
  ...
  it('should render context-saving badge for servers with contextSaving', () => {
    ...
    const csBadge = parentEl.querySelector('.korian-mcp-selector-cs-badge');
  ...
  it('should toggle server on mousedown and update display', () => {
    ...
    const item = parentEl.querySelector('.korian-mcp-selector-item');
...
describe('createInputToolbar', () => {
  ...
  it('should place the mode selector after the permission toggle in toolbar order', () => {
    ...
    const permissionIndex = parentEl.children.findIndex((child: any) => child.hasClass('korian-permission-toggle'));
    const modeIndex = parentEl.children.findIndex((child: any) => child.hasClass('korian-mode-selector'));
