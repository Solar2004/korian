...
describe('utils.ts', () => {
  ...
  describe('expandHomePath', () => {
    const envKey = 'KORIAN_TEST_PATH';
    const envValue = path.join(os.tmpdir(), 'korian-env');
    ...
    it('should leave unknown environment variables untouched', () => {
      expect(expandHomePath('%KORIAN_MISSING_VAR%')).toBe('%KORIAN_MISSING_VAR%');
      expect(expandHomePath('$KORIAN_MISSING_VAR')).toBe('$KORIAN_MISSING_VAR');
    });
  });
  ...
  describe('normalizePathForFilesystem', () => {
    ...
    it('expands environment variables before filesystem use', () => {
      const envKey = 'KORIAN_FS_TEST_PATH';
      const originalValue = process.env[envKey];
      process.env[envKey] = '/tmp/korian-test';

      try {
        expect(normalizePathForFilesystem(`$${envKey}/notes/file.md`)).toBe('/tmp/korian-test/notes/file.md');
      } finally {
        if (originalValue === undefined) {
          delete process.env[envKey];
        } else {
    ...
    it('handles chained home and environment variable expansions', () => {
      const envKey = 'KORIAN_TEST_SUBDIR';
      const originalValue = process.env[envKey];
