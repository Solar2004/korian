...
function clickButton(root: any, text: string): void {
  const buttons = findAllByClass(root, 'korian-instruction-btn');
...
describe('InstructionModal', () => {
  describe('onOpen', () => {
    it('renders the raw instruction text', () => {
      ...
      const originalEl = findByClass(contentEl, 'korian-instruction-original');
    ...
    it('starts in loading state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'korian-instruction-loading');
      expect(loadingEl).not.toBeNull();
      expect(loadingEl.hasClass('korian-hidden')).toBe(false);

      const clarificationEl = findByClass(contentEl, 'korian-instruction-clarification-section');
      expect(clarificationEl.hasClass('korian-hidden')).toBe(true);

      const confirmationEl = findByClass(contentEl, 'korian-instruction-confirmation-section');
      expect(confirmationEl.hasClass('korian-hidden')).toBe(true);
    });
    ...
    it('renders Cancel button in loading state', () => {
      ...
      const buttons = findAllByClass(contentEl, 'korian-instruction-btn');
  ...
  describe('showClarification', () => {
    it('transitions to clarification state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'korian-instruction-loading');
      expect(loadingEl.hasClass('korian-hidden')).toBe(true);

      const clarificationEl = findByClass(contentEl, 'korian-instruction-clarification-section');
      expect(clarificationEl.hasClass('korian-hidden')).toBe(false);
    });
    ...
    it('displays the clarification text', () => {
      ...
      const clarificationTextEl = findByClass(contentEl, 'korian-instruction-clarification');
  ...
  describe('showConfirmation', () => {
    it('transitions to confirmation state', () => {
      ...
      const confirmationEl = findByClass(contentEl, 'korian-instruction-confirmation-section');
      expect(confirmationEl.hasClass('korian-hidden')).toBe(false);
    });
    ...
    it('displays the refined instruction', () => {
      ...
      const refinedEl = findByClass(contentEl, 'korian-instruction-refined');
