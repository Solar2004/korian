...
describe('OpencodeChatRuntime', () => {
  ...
  it('marks missing saved sessions invalidated without creating a replacement command session', async () => {
    ...
    jest.spyOn(launchArtifacts, 'prepareOpencodeLaunchArtifacts').mockImplementation(async (params) => {
      ...
      return {
        configPath: '/tmp/korian-opencode-config.json',
        configContent: '{}\n',
        databasePath: '/persisted/opencode.db',
        launchKey: 'launch-key',
        systemPromptPath: '/tmp/korian-opencode-system.md',
      };
    });
  ...
  it('falls back to the managed YOLO mode when a saved custom mode is not managed by Korian', () => {
    const plugin = createMockPlugin({
      settings: {
        permissionMode: 'yolo',
        providerConfigs: {
        ...
        providerConfigs: {
          opencode: {
            availableModes: [
              ...
              { id: OPENCODE_SAFE_MODE_ID, name: 'korian-safe' },
              { id: 'plan', name: 'plan' },
            ],
            selectedMode: '',
          },
