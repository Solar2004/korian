...
describe('buildOpencodeManagedConfig', () => {
  it('pins OpenCode build, YOLO, safe, and plan prompts to the managed prompt file', () => {
    expect(buildOpencodeManagedConfig({}, '/vault/.korian/opencode/system.md', 'Yishen')).toEqual({
      $schema: 'https://opencode.ai/config.json',
      agent: {
        build: {
          prompt: '{file:/vault/.korian/opencode/system.md}',
        },
        [OPENCODE_YOLO_MODE_ID]: {
          mode: 'primary',
          permission: {
  ...
  it('can create a dedicated aux agent and default it for the process', () => {
    expect(buildOpencodeManagedConfig(
      {},
      '/vault/.korian/opencode/auxiliary/system.md',
      undefined,
      [{
        definition: {
          mode: 'primary',
      ...
      agent: {
        'korian-aux-readonly': {
          ...
          prompt: '{file:/vault/.korian/opencode/auxiliary/system.md}',
        },
      },
      default_agent: 'korian-aux-readonly',
    });
  });
  ...
  it('merges the user config instead of replacing it', () => {
    expect(buildOpencodeManagedConfig({
      ...
      username: 'Existing',
    }, '/vault/.korian/opencode/system.md')).toEqual({
      $schema: 'https://opencode.ai/config.json',
      agent: {
        build: {
          model: 'openai/gpt-5',
...
describe('prepareOpencodeLaunchArtifacts', () => {
  it('layers the managed prompt config on top of OPENCODE_CONFIG', async () => {
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'korian-opencode-artifacts-'));
    ...
    expect(result.configPath).toBe(path.join(tmpRoot, '.korian', 'opencode', 'config.json'));
    expect(result.systemPromptPath).toBe(path.join(tmpRoot, '.korian', 'opencode', 'system.md'));
