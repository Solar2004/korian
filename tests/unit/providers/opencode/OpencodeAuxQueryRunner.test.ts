...
function createMockPlugin(settings: Record<string, unknown> = {}) {
  return {
    ...
    app: {
      vault: {
        adapter: {
          basePath: '/tmp/korian-test-vault',
        },
      },
    },
  } as any;
...
describe('OpencodeAuxQueryRunner', () => {
  ...
  beforeEach(() => {
    ...
    mockPrepareOpencodeLaunchArtifacts.mockResolvedValue({
      configPath: '/tmp/korian-opencode-aux/config.json',
      configContent: '{"default_agent":"korian-aux-passive"}\n',
      databasePath: null,
      launchKey: 'launch-key',
      systemPromptPath: '/tmp/korian-opencode-aux/system.md',
    });
  });
  ...
  it('launches an auxiliary ACP session and streams assistant text', async () => {
    ...
    expect(mockPrepareOpencodeLaunchArtifacts).toHaveBeenCalledWith(expect.objectContaining({
      ...
      defaultAgentId: 'korian-aux-passive',
      managedAgents: [expect.objectContaining({ id: 'korian-aux-passive' })],
  ...
  it('rejects aux reads outside the workspace root', () => {
    ...
    (runner as any).sessionCwds.set('session-1', '/tmp/korian-test-vault');
    ...
    expect((runner as any).resolveSessionPath('session-1', '/tmp/korian-test-vault/notes/today.md')).toBe(
      '/tmp/korian-test-vault/notes/today.md',
    );
  });
});
