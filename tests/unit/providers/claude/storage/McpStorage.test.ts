...
describe('McpStorage', () => {
  describe('load', () => {
    ...
    it('loads servers with disabledTools from _korian metadata', async () => {
      const config = {
        mcpServers: {
    ...
    it('applies defaults when no _korian metadata exists', async () => {
      const config = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
    ...
    it('loads description from _korian metadata', async () => {
      const config = {
  ...
  describe('save', () => {
    it('saves disabledTools to _korian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      expect(saved._korian.servers.alpha.disabledTools).toEqual(['tool_a', 'tool_b']);
    });
    ...
    it('omits disabledTools from metadata when empty', async () => {
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      // No _korian since all fields are default
      expect(saved._korian).toBeUndefined();
    });

    it('preserves existing _korian metadata when saving', async () => {
      const existing = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._korian.customField).toBe('should be preserved');
      expect(saved._korian.servers.alpha.disabledTools).toEqual(['tool_a']);
    });
    ...
    it('saves description to _korian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._korian.servers.alpha.description).toBe('A test server');
    });

    it('stores enabled=false in _korian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._korian.servers.alpha.enabled).toBe(false);
    });

    it('stores contextSaving=false in _korian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._korian.servers.alpha.contextSaving).toBe(false);
    });

    it('removes _korian.servers when all metadata is default', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _korian: { servers: { alpha: { enabled: false } } },
      };
      const adapter = createMockAdapter({
        '.claude/mcp.json': JSON.stringify(existing),
      });
    ...
    it('preserves non-servers _korian fields when removing servers', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _korian: {
          customField: 'keep',
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._korian).toEqual({ customField: 'keep' });
    });
