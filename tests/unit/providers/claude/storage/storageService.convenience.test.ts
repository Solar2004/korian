...
describe('StorageService convenience methods', () => {
  ...
  const korianSettingsJson = JSON.stringify({
    userName: 'Test',
    model: 'haiku',
    permissionMode: 'yolo',
  });
  ...
  describe('updateKorianSettings', () => {
    it('updates partial korian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.korian/korian-settings.json': korianSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();

      await storage.updateKorianSettings({ userName: 'NewUser' });

      const saved = JSON.parse(files.get('.korian/korian-settings.json')!) as Record<string, unknown>;
      expect(saved.userName).toBe('NewUser');
    });
  });

  describe('saveKorianSettings', () => {
    it('saves full korian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.korian/korian-settings.json': korianSettingsJson,
        },
      ...
      const existing = await storage.loadKorianSettings();
      existing.userName = 'FullSave';
      await storage.saveKorianSettings(existing);
  ...
  describe('loadKorianSettings', () => {
    it('loads korian settings', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.korian/korian-settings.json': korianSettingsJson,
        },
      ...
      const settings = await storage.loadKorianSettings();
      expect(settings.userName).toBe('Test');
      expect(settings.model).toBe('haiku');
    });

    it('migrates legacy settings into .korian during initialization', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.claude/korian-settings.json': korianSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      ...
      expect(files.get('.korian/korian-settings.json')).toBeDefined();
      expect(files.has('.claude/korian-settings.json')).toBe(false);
    });
  });
