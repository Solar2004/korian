...
import type { KorianSettings } from '@/core/types/settings';
...
describe('types.ts', () => {
  describe('VIEW_TYPE_KORIAN', () => {
    it('should be defined as the correct view type', () => {
      expect(VIEW_TYPE_KORIAN).toBe('korian-view');
    });
  });
  ...
  describe('KorianSettings type', () => {
    it('should be assignable with valid settings', () => {
      const settings: KorianSettings = {
        userName: '',
        model: 'haiku',
