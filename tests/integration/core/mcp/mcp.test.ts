...
describe('McpStorage', () => {
  ...
  describe('load/save', () => {
    it('should preserve unknown top-level keys and merge _korian', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
    ...
    it('should keep existing _korian fields when metadata is defaulted', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const saved = JSON.parse(files.get(MCP_CONFIG_PATH) || '{}') as Record<string, unknown>;
      expect(saved._korian).toEqual({ extra: { keep: true } });
    ...
    it('should remove _korian when only servers metadata exists', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
