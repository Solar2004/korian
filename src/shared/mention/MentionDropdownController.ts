...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'korian-mention-dropdown',
      itemClassName: 'korian-mention-item',
      emptyClassName: 'korian-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'korian-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'korian-mention-icon' });
        switch (item.type) {
          case 'mcp-server':
        ...
        const textEl = itemEl.createSpan({ cls: 'korian-mention-text' });

        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'korian-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'korian-mention-name korian-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'korian-mention-name korian-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'korian-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'korian-mention-name korian-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'korian-mention-name korian-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'korian-mention-path' }).setText(item.path || item.name);
        }
      },
  ...
  private positionFixed(): void {
    ...
    dropdownEl.setCssProps({
      '--korian-fixed-dropdown-bottom': `${window.innerHeight - inputRect.top + 4}px`,
      '--korian-fixed-dropdown-left': `${inputRect.left}px`,
      '--korian-fixed-dropdown-width': `${Math.max(inputRect.width, 280)}px`,
    });
  }
