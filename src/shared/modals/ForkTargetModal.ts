...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('korian-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'korian-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'korian-fork-target-option', text: label });
