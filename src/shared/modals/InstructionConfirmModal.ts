...
export class InstructionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass('korian-instruction-modal');
    this.setTitle('Add custom instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'korian-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'korian-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'korian-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'korian-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'korian-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'korian-instruction-spinner' });
    this.loadingEl.createSpan({ text: 'Processing your instruction...' });

    // Clarification state (hidden initially)
    this.clarificationEl = this.contentSectionEl.createDiv({ cls: 'korian-instruction-clarification-section' });
    this.clarificationEl.addClass('korian-hidden');
    this.clarificationTextEl = this.clarificationEl.createDiv({ cls: 'korian-instruction-clarification' });

    const responseSection = this.clarificationEl.createDiv({ cls: 'korian-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'korian-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('korian-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({ cls: 'korian-instruction-confirmation-section' });
    this.confirmationEl.addClass('korian-hidden');

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'korian-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'korian-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'korian-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'korian-instruction-edit-container' });
    this.editContainerEl.addClass('korian-hidden');

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('korian-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'korian-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.korian-instruction-spinner');
  ...
  private showState(state: ModalState) {
    ...
    if (this.loadingEl) {
      this.loadingEl.toggleClass('korian-hidden', state !== 'loading');
    }
    if (this.clarificationEl) {
      this.clarificationEl.toggleClass('korian-hidden', state !== 'clarification');
    }
    if (this.confirmationEl) {
      this.confirmationEl.toggleClass('korian-hidden', state !== 'confirmation');
    }
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'korian-instruction-btn korian-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'korian-instruction-btn korian-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'korian-instruction-btn korian-instruction-edit-btn',
  ...
  private toggleEdit() {
    ...
    if (this.isEditing) {
      this.refinedDisplayEl?.addClass('korian-hidden');
      this.editContainerEl?.removeClass('korian-hidden');
      ...
      if (this.refinedDisplayEl) {
        ...
        this.refinedDisplayEl.removeClass('korian-hidden');
      }
      this.editContainerEl?.addClass('korian-hidden');
