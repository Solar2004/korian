...
Main sidebar chat interface. `KorianView` assembles tabs, controllers, renderers, and provider-backed services around the shared `ChatRuntime` boundary.
...
- `KorianView.onClose()` must abort active tabs and dispose runtimes
