...
export class InlineAskUserQuestion {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'korian-ask-question-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'korian-ask-inline-title' });
    ...
    if (!this.config.immediateSelect) {
      this.tabBar = this.rootEl.createDiv({ cls: 'korian-ask-tab-bar' });
      this.renderTabBar();
    }
    this.contentArea = this.rootEl.createDiv({ cls: 'korian-ask-content' });
  ...
  private renderTabBar(): void {
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tab = this.tabBar.createSpan({ cls: 'korian-ask-tab' });
      tab.createSpan({ text: this.questions[idx].header, cls: 'korian-ask-tab-label' });
      tab.createSpan({ text: answered ? ' \u2713' : '', cls: 'korian-ask-tab-tick' });
    ...
    const submitTab = this.tabBar.createSpan({ cls: 'korian-ask-tab' });
    submitTab.createSpan({ text: allAnswered ? '\u2713 ' : '', cls: 'korian-ask-tab-submit-check' });
    submitTab.createSpan({ text: 'Submit', cls: 'korian-ask-tab-label' });
  ...
  private renderQuestionTab(idx: number): void {
    ...
    this.contentArea.createDiv({
      text: q.question,
      cls: 'korian-ask-question-text',
    });

    const listEl = this.contentArea.createDiv({ cls: 'korian-ask-list' });
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      const row = listEl.createDiv({ cls: 'korian-ask-item' });
      if (isFocused) row.addClass('is-focused');
      if (isSelected) row.addClass('is-selected');

      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'korian-ask-cursor' });
      row.createSpan({ text: `${optIdx + 1}. `, cls: 'korian-ask-item-num' });

      if (isMulti) {
        this.renderMultiSelectCheckbox(row, isSelected);
      }

      const labelBlock = row.createDiv({ cls: 'korian-ask-item-content' });
      const labelRow = labelBlock.createDiv({ cls: 'korian-ask-label-row' });
      labelRow.createSpan({ text: option.label, cls: 'korian-ask-item-label' });

      if (!isMulti && isSelected) {
        labelRow.createSpan({ text: ' \u2713', cls: 'korian-ask-check-mark' });
      }

      if (option.description) {
        labelBlock.createDiv({ text: option.description, cls: 'korian-ask-item-desc' });
      }
    ...
    if (this.canShowCustomInputForQuestion(q)) {
      ...
      const customRow = listEl.createDiv({ cls: 'korian-ask-item korian-ask-custom-item' });
      if (customFocused) customRow.addClass('is-focused');

      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'korian-ask-cursor' });
      customRow.createSpan({ text: `${customIdx + 1}. `, cls: 'korian-ask-item-num' });
  ...
  private renderSubmitTab(): void {
    this.contentArea.createDiv({
      ...
      cls: 'korian-ask-review-title',
    });

    const reviewEl = this.contentArea.createDiv({ cls: 'korian-ask-review' });
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      pairEl.createDiv({ text: `${idx + 1}.`, cls: 'korian-ask-review-num' });
      ...
      bodyEl.createDiv({
        ...
        cls: answerText ? 'korian-ask-review-a-text' : 'korian-ask-review-empty',
      });
    ...
    const actionsEl = this.contentArea.createDiv({ cls: 'korian-ask-list' });
    const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));

    const submitRow = actionsEl.createDiv({ cls: 'korian-ask-item' });
    if (this.focusedItemIndex === 0) submitRow.addClass('is-focused');
    if (!allAnswered) submitRow.addClass('is-disabled');
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'korian-ask-cursor' });
    submitRow.createSpan({ text: '1. ', cls: 'korian-ask-item-num' });
    submitRow.createSpan({ text: 'Submit answers', cls: 'korian-ask-item-label' });
    ...
    const cancelRow = actionsEl.createDiv({ cls: 'korian-ask-item' });
    if (this.focusedItemIndex === 1) cancelRow.addClass('is-focused');
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'korian-ask-cursor' });
    cancelRow.createSpan({ text: '2. ', cls: 'korian-ask-item-num' });
    cancelRow.createSpan({ text: 'Cancel', cls: 'korian-ask-item-label' });
  ...
  private renderMultiSelectCheckbox(parent: HTMLElement, checked: boolean): void {
    parent.createSpan({
      text: checked ? '[\u2713] ' : '[ ] ',
      cls: `korian-ask-check${checked ? ' is-checked' : ''}`,
    });
  }
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.korian-ask-check');
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.korian-ask-label-row');
        const existingMark = item.querySelector('.korian-ask-check-mark');
  ...
  private updateFocusIndicator(): void {
    for (let i = 0; i < this.currentItems.length; i++) {
      const item = this.currentItems[i];
      const cursor = item.querySelector('.korian-ask-cursor');
  ...
  private updateTabIndicators(): void {
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tick = tab.querySelector('.korian-ask-tab-tick');
      const answered = this.isQuestionAnswered(idx);
    ...
    if (submitTab) {
      const submitCheck = submitTab.querySelector('.korian-ask-tab-submit-check');
      const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (this.focusedItemIndex < q.options.length) {
          ...
          const input = customRow?.querySelector('.korian-ask-custom-text') as HTMLInputElement;
          input?.focus();
        }
        break;
    }
