...
export class InlinePlanApproval {
  ...
  render(): void {
    ...
    this.rootEl.createDiv({ cls: 'korian-plan-inline-title', text: 'Plan complete' });

    const actionsEl = this.rootEl.createDiv({ cls: 'korian-ask-list' });

    // 1. Implement
    const implementRow = actionsEl.createDiv({ cls: 'korian-ask-item' });
    implementRow.addClass('is-focused');
    implementRow.createSpan({ text: '\u203A', cls: 'korian-ask-cursor' });
    implementRow.createSpan({ text: '1. ', cls: 'korian-ask-item-num' });
    implementRow.createSpan({ text: 'Implement', cls: 'korian-ask-item-label' });
    ...
    const reviseRow = actionsEl.createDiv({ cls: 'korian-ask-item korian-ask-custom-item' });
    reviseRow.createSpan({ text: '\u00A0', cls: 'korian-ask-cursor' });
    reviseRow.createSpan({ text: '2. ', cls: 'korian-ask-item-num' });
    ...
    cancelRow.createSpan({ text: '\u00A0', cls: 'korian-ask-cursor' });
    cancelRow.createSpan({ text: '3. ', cls: 'korian-ask-item-num' });
  ...
  private updateFocus(): void {
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (i === this.focusedIndex) {
        ...
        if (item.hasClass('korian-ask-custom-item') && this.isInputFocused) {
          const input = item.querySelector('.korian-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.blur();
            this.isInputFocused = false;
