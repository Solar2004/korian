...
export class InlineExitPlanMode {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'korian-plan-approval-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'korian-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'korian-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'korian-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'korian-plan-content-preview korian-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'korian-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'korian-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'korian-plan-permissions-list' });
      for (const perm of allowedPrompts) {
        listEl.createEl('li', { text: perm.prompt });
      }
    }

    const actionsEl = this.rootEl.createDiv({ cls: 'korian-ask-list' });

    const newSessionRow = actionsEl.createDiv({ cls: 'korian-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'korian-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'korian-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'korian-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'korian-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'korian-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'korian-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'korian-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'korian-ask-item korian-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'korian-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'korian-ask-item-num' });
    ...
    this.rootEl.createDiv({ text: HINTS_TEXT, cls: 'korian-ask-hints' });
  ...
  private updateFocus(): void {
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (i === this.focusedIndex) {
        ...
        if (item.hasClass('korian-ask-custom-item')) {
          const input = item.querySelector('.korian-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.focus();
