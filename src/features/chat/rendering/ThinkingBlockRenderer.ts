...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'korian-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'korian-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'korian-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'korian-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.korian-thinking-header');
  if (header) {
