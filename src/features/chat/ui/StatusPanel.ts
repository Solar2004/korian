...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.panelEl.className = 'korian-status-panel';

    // Bash output container - hidden by default
    this.bashOutputContainerEl = ownerDocument.createElement('div');
    this.bashOutputContainerEl.className = 'korian-status-panel-bash korian-hidden';

    this.bashHeaderEl = ownerDocument.createElement('div');
    this.bashHeaderEl.className = 'korian-tool-header korian-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'korian-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'korian-status-panel-todos korian-hidden';
    this.panelEl.appendChild(this.todoContainerEl);

    // Todo header (collapsed view)
    this.todoHeaderEl = ownerDocument.createElement('div');
    this.todoHeaderEl.className = 'korian-status-panel-header';
    ...
    this.todoContentEl.className = 'korian-status-panel-content korian-todo-list-container korian-hidden';
  ...
  updateTodos(todos: TodoItem[] | null): void {
    ...
    if (!todos || todos.length === 0) {
      this.todoContainerEl.addClass('korian-hidden');
      this.todoHeaderEl.empty();
      this.todoContentEl.empty();
      return;
    }

    this.todoContainerEl.removeClass('korian-hidden');
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'korian-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'korian-status-panel-current';
  ...
  private updateTodoDisplay(): void {
    ...
    // Show/hide content
    this.todoContentEl.toggleClass('korian-hidden', !this.isTodoExpanded);
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    if (this.currentBashOutputs.size === 0) {
      this.bashOutputContainerEl.addClass('korian-hidden');
      return;
    }

    this.bashOutputContainerEl.removeClass('korian-hidden');
    ...
    headerIconEl.className = 'korian-tool-icon';
    ...
    headerLabelEl.className = 'korian-tool-label';
    ...
    previewEl.className = 'korian-tool-current';
    previewEl.classList.toggle('korian-hidden', !this.isBashExpanded);
    this.bashHeaderEl.appendChild(previewEl);

    const summaryStatusEl = ownerDocument.createElement('span');
    summaryStatusEl.className = 'korian-tool-status';
    if (!this.isBashExpanded && latest) {
      ...
      summaryStatusEl.classList.add('korian-hidden');
    }
    ...
    actionsEl.className = 'korian-status-panel-bash-actions';
    ...
    this.bashContentEl.toggleClass('korian-hidden', !this.isBashExpanded);
  ...
  private renderBashEntry(info: PanelBashOutput, ownerDocument: Document): HTMLElement {
    ...
    entryEl.className = 'korian-tool-call korian-status-panel-bash-entry';

    const entryHeaderEl = ownerDocument.createElement('div');
    entryHeaderEl.className = 'korian-tool-header';
    ...
    entryIconEl.className = 'korian-tool-icon';
    ...
    entryLabelEl.className = 'korian-tool-label';
    ...
    entryStatusEl.className = 'korian-tool-status';
    ...
    contentEl.className = 'korian-tool-content';
    const isEntryExpanded = this.bashEntryExpanded.get(info.id) ?? true;
    contentEl.classList.toggle('korian-hidden', !isEntryExpanded);
    ...
    rowEl.className = 'korian-tool-result-row';

    const textEl = ownerDocument.createElement('span');
    textEl.className = 'korian-tool-result-text';
  ...
  private appendActionButton(
    ...
    el.className = `korian-status-panel-bash-action korian-status-panel-bash-action-${name}`;
