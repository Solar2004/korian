...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.korian-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--korian-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--korian-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--korian-textarea-min-height': `${minHeight}px`,
    '--korian-textarea-max-height': `${maxHeight}px`,
  });
}
