...
export class FileChipsView {
  ...
  constructor(containerEl: HTMLElement, callbacks: FileChipsViewCallbacks) {
    ...
    this.fileIndicatorEl = this.containerEl.createDiv({ cls: 'korian-file-indicator' });
    if (firstChild) {
  ...
  renderCurrentNote(filePath: string | null): void {
    ...
    if (!filePath) {
      this.fileIndicatorEl.removeClass('korian-visible-flex');
      this.fileIndicatorEl.addClass('korian-hidden');
      return;
    }

    this.fileIndicatorEl.addClass('korian-visible-flex');
    this.fileIndicatorEl.removeClass('korian-hidden');
  ...
  private renderFileChip(filePath: string, onRemove: () => void): void {
    const chipEl = this.fileIndicatorEl.createDiv({ cls: 'korian-file-chip' });

    const iconEl = chipEl.createSpan({ cls: 'korian-file-chip-icon' });
    setIcon(iconEl, 'file-text');

    const normalizedPath = filePath.replace(/\\/g, '/');
    const filename = normalizedPath.split('/').pop() || filePath;
    const nameEl = chipEl.createSpan({ cls: 'korian-file-chip-name' });
    nameEl.setText(filename);
    nameEl.setAttribute('title', filePath);

    const removeEl = chipEl.createSpan({ cls: 'korian-file-chip-remove' });
    removeEl.setText('\u00D7');
    removeEl.setAttribute('aria-label', 'Remove');

    chipEl.addEventListener('click', (e) => {
      if (!(e.target as HTMLElement).closest('.korian-file-chip-remove')) {
