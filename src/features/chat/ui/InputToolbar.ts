...
export class ModelSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    ...
    this.container = parentEl.createDiv({ cls: 'korian-model-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'korian-model-btn' });
    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'korian-model-dropdown' });
    this.renderOptions();
  }
  ...
  updateDisplay() {
    ...
    const labelEl = this.buttonEl.createSpan({ cls: 'korian-model-label' });
  ...
  renderOptions() {
    ...
    for (const model of reversed) {
      if (model.group && model.group !== lastGroup) {
        const separator = this.dropdownEl.createDiv({ cls: 'korian-model-group' });
        separator.setText(model.group);
        lastGroup = model.group;
      }

      const option = this.dropdownEl.createDiv({ cls: 'korian-model-option' });
      ...
      if (icon) {
        option.appendChild(createProviderIconSvg(icon, {
          className: 'korian-model-provider-icon',
          height: 12,
...
export class ModeSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'korian-mode-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'korian-mode-label' });
    this.toggleEl = this.container.createDiv({ cls: 'korian-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (!selectorConfig || selectorConfig.options.length !== 2) {
      this.container.addClass('korian-hidden');
      return;
    }

    this.container.removeClass('korian-hidden');
...
export class ThinkingBudgetSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'korian-thinking-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.effortEl = this.container.createDiv({ cls: 'korian-thinking-effort' });
    const effortLabel = this.effortEl.createSpan({ cls: 'korian-thinking-label-text' });
    effortLabel.setText('Effort:');
    this.effortGearsEl = this.effortEl.createDiv({ cls: 'korian-thinking-gears' });

    // Legacy budget selector (for custom models)
    this.budgetEl = this.container.createDiv({ cls: 'korian-thinking-budget' });
    const budgetLabel = this.budgetEl.createSpan({ cls: 'korian-thinking-label-text' });
    budgetLabel.setText('Thinking:');
    this.budgetGearsEl = this.budgetEl.createDiv({ cls: 'korian-thinking-gears' });
  ...
  private renderEffortGears() {
    ...
    const currentEl = this.effortGearsEl.createDiv({ cls: 'korian-thinking-current' });
    currentEl.setText(currentInfo?.label || options[0]?.label || 'High');

    const optionsEl = this.effortGearsEl.createDiv({ cls: 'korian-thinking-options' });

    for (const effort of [...options].reverse()) {
      const gearEl = optionsEl.createDiv({ cls: 'korian-thinking-gear' });
  ...
  private renderBudgetGears() {
    ...
    const currentEl = this.budgetGearsEl.createDiv({ cls: 'korian-thinking-current' });
    currentEl.setText(currentBudgetInfo?.label || options[0]?.label || 'Off');

    const optionsEl = this.budgetGearsEl.createDiv({ cls: 'korian-thinking-options' });
  ...
  updateDisplay() {
    ...
    if (capabilities.reasoningControl === 'none') {
      this.effortEl?.addClass('korian-hidden');
      this.budgetEl?.addClass('korian-hidden');
      return;
    }
    ...
    if (this.effortEl) {
      this.effortEl.toggleClass('korian-hidden', !adaptive);
    }
    if (this.budgetEl) {
      this.budgetEl.toggleClass('korian-hidden', adaptive);
    }
...
export class PermissionToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'korian-permission-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'korian-permission-label' });
    this.toggleEl = this.container.createDiv({ cls: 'korian-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (canShowPlan && planValue && mode === planValue) {
      this.toggleEl.addClass('korian-hidden');
      this.labelEl.setText(planLabel);
      this.labelEl.addClass('plan-active');
    } else {
      this.toggleEl.removeClass('korian-hidden');
...
export class ServiceTierToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'korian-service-tier-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'korian-service-tier-button' });
    this.iconEl = this.buttonEl.createSpan({ cls: 'korian-service-tier-icon' });
...
export class ExternalContextSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'korian-external-context-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'korian-external-context-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'korian-external-context-icon' });
    setIcon(this.iconEl, 'folder');

    this.badgeEl = iconWrapper.createDiv({ cls: 'korian-external-context-badge' });
    ...
    this.dropdownEl = this.container.createDiv({ cls: 'korian-external-context-dropdown' });
    this.renderDropdown();
  }
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'korian-external-context-header' });
    headerEl.setText('External contexts');

    // Path list
    const listEl = this.dropdownEl.createDiv({ cls: 'korian-external-context-list' });

    if (this.externalContextPaths.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'korian-external-context-empty' });
      emptyEl.setText('Click folder icon to add');
    } else {
      for (const pathStr of this.externalContextPaths) {
        const itemEl = listEl.createDiv({ cls: 'korian-external-context-item' });

        const pathTextEl = itemEl.createSpan({ cls: 'korian-external-context-text' });
        ...
        const lockBtn = itemEl.createSpan({ cls: 'korian-external-context-lock' });
        if (isPersistent) {
        ...
        const removeBtn = itemEl.createSpan({ cls: 'korian-external-context-remove' });
        setIcon(removeBtn, 'x');
...
export class McpServerSelector {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'korian-mcp-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'korian-mcp-selector-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'korian-mcp-selector-icon' });
    appendMcpIcon(this.iconEl);

    this.badgeEl = iconWrapper.createDiv({ cls: 'korian-mcp-selector-badge' });

    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'korian-mcp-selector-dropdown' });
    this.renderDropdown();
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'korian-mcp-selector-header' });
    headerEl.setText('Mcp servers');

    // Server list
    const listEl = this.dropdownEl.createDiv({ cls: 'korian-mcp-selector-list' });

    const allServers = this.mcpManager?.getServers() || [];
    const servers = allServers.filter(s => s.enabled);

    if (servers.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'korian-mcp-selector-empty' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'korian-mcp-selector-item' });
    ...
    // Checkbox
    const checkEl = itemEl.createDiv({ cls: 'korian-mcp-selector-check' });
    if (isEnabled) {
      appendCheckIcon(checkEl);
    }

    // Info
    const infoEl = itemEl.createDiv({ cls: 'korian-mcp-selector-item-info' });

    const nameEl = infoEl.createSpan({ cls: 'korian-mcp-selector-item-name' });
    nameEl.setText(server.name);

    // Badges
    if (server.contextSaving) {
      const csEl = infoEl.createSpan({ cls: 'korian-mcp-selector-cs-badge' });
      csEl.setText('@');
  ...
  private toggleServer(name: string, itemEl: HTMLElement) {
    ...
    const checkEl = itemEl.querySelector<HTMLElement>('.korian-mcp-selector-check');
...
export class ContextUsageMeter {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'korian-context-meter' });
    this.render();
    // Initially hidden
    this.container.addClass('korian-hidden');
  }
  ...
  setVisible(visible: boolean): void {
    this.container.toggleClass('korian-hidden', !visible);
  }
  ...
  private render() {
    ...
    const gaugeEl = this.container.createDiv({ cls: 'korian-context-meter-gauge' });
    ...
    backgroundPath.classList.add('korian-meter-bg');
    ...
    fillPath.classList.add('korian-meter-fill');
    ...
    this.percentEl = this.container.createSpan({ cls: 'korian-context-meter-percent' });
  }
