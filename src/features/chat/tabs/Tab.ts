...
import type { ChatMessage, KorianSettings, Conversation, StreamChunk } from '../../../core/types';
...
function shouldSendMessageFromEnterKey(
  e: KeyboardEvent,
  settings: Pick<KorianSettings, 'requireCommandOrControlEnterToSend'>,
): boolean {
...
function refreshTabProviderUI(tab: TabData, plugin: KorianPlugin): void {
 ...
 */
function applyProviderUIGating(tab: TabData, plugin: KorianPlugin): void {
  const capabilities = getTabCapabilities(tab, plugin);
...
function ensureTitleGenerationService(tab: TabData, plugin: KorianPlugin): void {
 ...
 */
export function onProviderAvailabilityChanged(tab: TabData, plugin: KorianPlugin): void {
...
export function createTab(options: TabCreateOptions): TabData {
  ...
  const contentEl = containerEl.createDiv({ cls: 'korian-tab-content korian-hidden' });
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  const messagesWrapperEl = contentEl.createDiv({ cls: 'korian-messages-wrapper' });
  const messagesEl = messagesWrapperEl.createDiv({ cls: 'korian-messages' });
  const welcomeEl = messagesEl.createDiv({ cls: 'korian-welcome' });
  const statusPanelContainerEl = contentEl.createDiv({ cls: 'korian-status-panel-container' });
  const inputContainerEl = contentEl.createDiv({ cls: 'korian-input-container' });
  const queueIndicatorEl = inputContainerEl.createDiv({ cls: 'korian-input-queue-row' });
  const navRowEl = inputContainerEl.createDiv({ cls: 'korian-input-nav-row' });
  const inputWrapper = inputContainerEl.createDiv({ cls: 'korian-input-wrapper' });
  const contextRowEl = inputWrapper.createDiv({ cls: 'korian-context-row' });
...
function initializeContextManagers(tab: TabData, plugin: KorianPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: KorianPlugin): void {
  const { dom } = tab;
...
function initializeInputToolbar(
  ...
  const inputToolbar = dom.inputWrapper.createDiv({ cls: 'korian-input-toolbar' });
...
export function initializeTabUI(
  ...
  dom.selectionIndicatorEl = dom.contextRowEl.createDiv({ cls: 'korian-selection-indicator korian-hidden' });

  dom.browserIndicatorEl = dom.contextRowEl.createDiv({ cls: 'korian-browser-selection-indicator korian-hidden' });

  dom.canvasIndicatorEl = dom.contextRowEl.createDiv({ cls: 'korian-canvas-indicator korian-hidden' });
 ...
 */
function resolveForkSource(tab: TabData, plugin: KorianPlugin): ForkSource | null {
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: KorianPlugin): void {
...
export function activateTab(tab: TabData): void {
  tab.dom.contentEl.removeClass('korian-hidden');
...
export function deactivateTab(tab: TabData): void {
  tab.dom.contentEl.addClass('korian-hidden');
 ...
 */
export function getTabTitle(tab: TabData, plugin: KorianPlugin): string {
...
export function setupServiceCallbacks(tab: TabData, plugin: KorianPlugin): void {
...
export function updatePlanModeUI(tab: TabData, plugin: KorianPlugin, mode: string): void {
  const providerId = getTabProviderId(tab, plugin);
  const snapshot = getTabSettingsSnapshot(tab, plugin);
  const uiConfig = ProviderRegistry.getChatUIConfig(providerId);
  if (uiConfig.applyPermissionMode) {
