...
export class TabBar {
  ...
  private build(): void {
    this.containerEl.addClass('korian-tab-badges');
  }
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    let stateClass = 'korian-tab-badge-idle';
    if (item.isActive) {
      stateClass = 'korian-tab-badge-active';
    } else if (item.needsAttention) {
      stateClass = 'korian-tab-badge-attention';
    } else if (item.isStreaming) {
      stateClass = 'korian-tab-badge-streaming';
    }

    const badgeEl = this.containerEl.createDiv({
      cls: `korian-tab-badge ${stateClass}`,
  ...
  destroy(): void {
    this.containerEl.empty();
    this.containerEl.removeClass('korian-tab-badges');
  }
}
