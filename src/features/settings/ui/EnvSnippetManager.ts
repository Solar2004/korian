...
import type { KorianView } from '../../chat/KorianView';
...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('korian-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      if (uniqueModelIds.size === 0) {
        contextLimitsContainer.addClass('korian-hidden');
        return;
      }

      contextLimitsContainer.removeClass('korian-hidden');
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'korian-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'korian-snippet-limit-model' });
        row.createSpan({ cls: 'korian-snippet-limit-spacer' });

        const aliasInput = row.createEl('input', {
          type: 'text',
          placeholder: t('settings.customModelAliases.placeholder'),
          cls: 'korian-snippet-alias-input',
        });
        ...
        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'korian-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('korian-env-snippet-setting');
    envVarsSetting.controlEl.addClass('korian-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'korian-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'korian-snippet-buttons' });

    const cancelBtn = buttonContainer.createEl('button', {
      text: t('settings.envSnippets.modal.cancel'),
      cls: 'korian-cancel-btn'
    });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'korian-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'korian-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'korian-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'korian-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'korian-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'korian-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'korian-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'korian-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'korian-snippet-actions' });
      ...
      const deleteBtn = actionsEl.createEl('button', {
        cls: 'korian-settings-action-btn korian-settings-delete-btn',
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const view = this.plugin.app.workspace.getLeavesOfType('korian-view')[0]?.view as KorianView | undefined;
  ...
  private syncTextareaValue(scope: EnvironmentScope, value: string): void {
    const selector = `.korian-settings-env-textarea[data-env-scope="${scope}"]`;
