  ...
  render(container, context) {
    ...
    if (isWindowsHost) {
      new Setting(container)
        ...
        .setDesc('How Korian should launch Codex on Windows. Native Windows uses a Windows executable path. WSL launches the Linux CLI inside a selected distro.')
    ...
    const refreshInstallationMethodUI = (): void => {
      ...
      if (wslDistroSettingEl) {
        wslDistroSettingEl.toggleClass('korian-hidden', installationMethod !== 'wsl');
      }
      if (wslDistroInputEl) {
    ...
    if (codexCatalog) {
      ...
      const skillsDesc = container.createDiv({ cls: 'korian-sp-settings-desc' });
      ...
      const skillsContainer = container.createDiv({ cls: 'korian-slash-commands-container' });
    ...
    const subagentDesc = container.createDiv({ cls: 'korian-sp-settings-desc' });
    ...
    const subagentContainer = container.createDiv({ cls: 'korian-slash-commands-container' });
    ...
    const mcpNotice = container.createDiv({ cls: 'korian-mcp-settings-desc' });
    ...
    mcpDesc.appendText(' and they will be available in Korian. ');
    mcpDesc.createEl('a', {
      text: 'Learn more',
