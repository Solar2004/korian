...
export class PluginSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'korian-plugin-header' });
    headerEl.createSpan({ text: 'Claude Code Plugins', cls: 'korian-plugin-label' });
    ...
    if (plugins.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'korian-plugin-empty' });
    ...
    const listEl = this.containerEl.createDiv({ cls: 'korian-plugin-list' });

    if (projectPlugins.length > 0) {
      const sectionHeader = listEl.createDiv({ cls: 'korian-plugin-section-header' });
  ...
  private renderPluginItem(listEl: HTMLElement, plugin: PluginInfo) {
    const itemEl = listEl.createDiv({ cls: 'korian-plugin-item' });
    if (!plugin.enabled) {
      itemEl.addClass('korian-plugin-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'korian-plugin-status' });
    if (plugin.enabled) {
      statusEl.addClass('korian-plugin-status-enabled');
    } else {
      statusEl.addClass('korian-plugin-status-disabled');
    }

    const infoEl = itemEl.createDiv({ cls: 'korian-plugin-info' });

    const nameRow = infoEl.createDiv({ cls: 'korian-plugin-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'korian-plugin-name' });
    nameEl.setText(plugin.name);

    const actionsEl = itemEl.createDiv({ cls: 'korian-plugin-actions' });

    const toggleBtn = actionsEl.createEl('button', {
      cls: 'korian-plugin-action-btn',
