...
export class SlashCommandModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('korian-sp-modal');
    ...
    const updateSkillOnlyFields = () => {
      ...
      disableUserSetting.settingEl.toggleClass('korian-hidden', !isSkillType);
      if (!isSkillType) {
    ...
    const details = contentEl.createEl('details', { cls: 'korian-sp-advanced-section' });
    details.createEl('summary', {
      text: 'Advanced options',
      cls: 'korian-sp-advanced-summary',
    });
    if (
      ...
      .addToggle(toggle => {
        toggle.setValue(contextValue === 'fork')
          .onChange(value => {
            ...
            agentSetting.settingEl.toggleClass('korian-hidden', !value);
          });
      });
    ...
    agentSetting.settingEl.toggleClass('korian-hidden', contextValue !== 'fork');
    ...
    const buttonContainer = contentEl.createDiv({ cls: 'korian-sp-modal-buttons' });
...
export class SlashCommandSettings {
  ...
  private renderUnavailable(): void {
    ...
    const emptyEl = this.containerEl.createDiv({ cls: 'korian-sp-empty-state' });
  ...
  private render(): void {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'korian-sp-header' });
    headerEl.createSpan({ text: t('settings.slashCommands.name'), cls: 'korian-sp-label' });

    const actionsEl = headerEl.createDiv({ cls: 'korian-sp-header-actions' });
    ...
    const listEl = this.containerEl.createDiv({ cls: 'korian-sp-list' });
  ...
  private renderCommandItem(listEl: HTMLElement, cmd: ProviderCommandEntry): void {
    const itemEl = listEl.createDiv({ cls: 'korian-sp-item' });

    const infoEl = itemEl.createDiv({ cls: 'korian-sp-info' });

    const headerRow = infoEl.createDiv({ cls: 'korian-sp-item-header' });

    const nameEl = headerRow.createSpan({ cls: 'korian-sp-item-name' });
    nameEl.setText(`/${cmd.name}`);

    if (isSkillEntry(cmd)) {
      headerRow.createSpan({ text: 'skill', cls: 'korian-slash-item-badge' });
    }

    if (cmd.argumentHint) {
      const hintEl = headerRow.createSpan({ cls: 'korian-slash-item-hint' });
      hintEl.setText(cmd.argumentHint);
    }

    if (cmd.description) {
      const descEl = infoEl.createDiv({ cls: 'korian-sp-item-desc' });
      descEl.setText(cmd.description);
    }

    const actionsEl = itemEl.createDiv({ cls: 'korian-sp-item-actions' });
