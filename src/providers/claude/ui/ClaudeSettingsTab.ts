...
export const claudeSettingsTabRenderer: ProviderSettingsTabRenderer = {
  render(container, context) {
    ...
    const slashCommandsDesc = container.createDiv({ cls: 'korian-sp-settings-desc' });
    ...
    const slashCommandsContainer = container.createDiv({ cls: 'korian-slash-commands-container' });
    ...
    const agentsDesc = container.createDiv({ cls: 'korian-sp-settings-desc' });
    agentsDesc.createEl('p', {
      text: t('settings.subagents.desc'),
      cls: 'setting-item-description',
    });

    const agentsContainer = container.createDiv({ cls: 'korian-agents-container' });
    ...
    const mcpDesc = container.createDiv({ cls: 'korian-mcp-settings-desc' });
    mcpDesc.createEl('p', {
      text: t('settings.mcpServers.desc'),
      cls: 'setting-item-description',
    });

    const mcpContainer = container.createDiv({ cls: 'korian-mcp-container' });
    ...
    const pluginsDesc = container.createDiv({ cls: 'korian-plugin-settings-desc' });
    pluginsDesc.createEl('p', {
      text: t('settings.plugins.desc'),
      cls: 'setting-item-description',
    });

    const pluginsContainer = container.createDiv({ cls: 'korian-plugins-container' });
    ...
    new Setting(container)
      ...
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (value) => {
            bangBashValidationEl.toggleClass('korian-hidden', true);
            if (value) {
              ...
              if (!nodePath) {
                ...
                bangBashValidationEl.toggleClass('korian-hidden', false);
    ...
    const bangBashValidationEl = container.createDiv({
      cls: 'korian-bang-bash-validation korian-setting-validation korian-setting-validation-error korian-hidden',
    });
  },
};
