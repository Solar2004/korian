...
export const opencodeSettingsTabRenderer: ProviderSettingsTabRenderer = {
  render(container, context) {
    ...
    cliPathSetting.addText((text) => {
      ...
      text.inputEl.addClass('korian-settings-cli-path-input');
    ...
    const pickerEl = container.createDiv({ cls: 'korian-opencode-model-picker' });

    let searchQuery = '';
    let providerFilter = ALL_PROVIDERS_KEY;

    const summaryEl = pickerEl.createDiv({ cls: 'korian-opencode-model-picker-summary' });
    const selectedEl = pickerEl.createDiv({ cls: 'korian-opencode-model-picker-selected' });
    const catalogEl = pickerEl.createEl('details', { cls: 'korian-opencode-model-picker-catalog' });
    ...
    const catalogSummaryEl = catalogEl.createEl('summary', {
      cls: 'korian-opencode-model-picker-catalog-summary',
    });
    catalogSummaryEl.createSpan({
      cls: 'korian-opencode-model-picker-catalog-caret',
      text: '▸',
    });
    catalogSummaryEl.createSpan({
      cls: 'korian-opencode-model-picker-catalog-title',
      text: 'Browse models',
    });
    const catalogSummaryCountEl = catalogSummaryEl.createSpan({
      cls: 'korian-opencode-model-picker-catalog-count',
    });

    const controlsEl = catalogEl.createDiv({ cls: 'korian-opencode-model-picker-controls' });

    const searchInput = controlsEl.createEl('input', {
      cls: 'korian-opencode-model-picker-search',
      type: 'search',
    });
    ...
    const providerSelectEl = controlsEl.createEl('select', {
      cls: 'korian-opencode-model-picker-provider',
    });
    ...
    const listEl = catalogEl.createDiv({ cls: 'korian-opencode-model-picker-list' });
    ...
    const renderSummary = (): void => {
      ...
      summaryEl.createSpan({
        cls: 'korian-opencode-model-picker-summary-value',
    ...
    const renderSelected = (): void => {
      ...
      if (current.visibleModels.length === 0) {
        selectedEl.toggleClass('korian-hidden', true);
        return;
      }

      selectedEl.toggleClass('korian-hidden', false);
      const enrichedByRawId = new Map(
        getEnrichedModels().map((model) => [model.rawId, model] as const),
      );

      const headerEl = selectedEl.createDiv({ cls: 'korian-opencode-model-picker-selected-header' });
      headerEl.createEl('span', {
        cls: 'korian-opencode-model-picker-selected-label',
        text: `Selected (${current.visibleModels.length})`,
      });
      const clearAllBtn = headerEl.createEl('button', {
        cls: 'korian-opencode-model-picker-selected-clear',
        text: 'Clear all',
      });
      ...
      const rowsEl = selectedEl.createDiv({ cls: 'korian-opencode-model-picker-selected-rows' });
      ...
      for (const rawId of current.visibleModels) {
        ...
        const rowEl = rowsEl.createDiv({ cls: 'korian-opencode-model-picker-selected-row' });
        if (enriched && !enriched.isAvailable) {
          rowEl.classList.add('korian-opencode-model-picker-selected-row--unavailable');
        }

        const infoEl = rowEl.createDiv({ cls: 'korian-opencode-model-picker-selected-info' });
        const titleEl = infoEl.createDiv({ cls: 'korian-opencode-model-picker-selected-title' });
        if (enriched) {
          titleEl.createEl('span', {
            cls: 'korian-opencode-model-picker-selected-badge',
            text: enriched.providerLabel,
          });
          titleEl.createEl('span', {
            cls: 'korian-opencode-model-picker-selected-name',
        ...
        if (enriched && !enriched.isAvailable) {
          infoEl.createEl('div', {
            cls: 'korian-opencode-model-picker-selected-unavailable',
            text: 'Not currently reported by OpenCode',
          });
        }

        infoEl.createEl('div', {
          cls: 'korian-opencode-model-picker-selected-id',
          text: rawId,
        });

        const controlsEl = rowEl.createDiv({ cls: 'korian-opencode-model-picker-selected-controls' });
        const aliasInput = controlsEl.createEl('input', {
          cls: 'korian-opencode-model-picker-selected-alias',
          type: 'text',
        });
        ...
        const removeBtn = controlsEl.createEl('button', {
          cls: 'korian-opencode-model-picker-selected-remove',
          text: '×',
        });
    ...
    const renderList = (): void => {
      ...
      if (filtered.length === 0) {
        const emptyEl = listEl.createDiv({ cls: 'korian-opencode-model-picker-empty' });
        ...
        if (loadingModelCatalog) {
          ...
          emptyText = 'Start OpenCode once to load its model catalog. Korian will then let you pick visible models.';
        }
      ...
      for (const model of filtered) {
        const rowEl = listEl.createEl('label', { cls: 'korian-opencode-model-picker-row' });
        const isSelected = selectedIds.has(model.rawId);
        if (isSelected) {
          rowEl.classList.add('korian-opencode-model-picker-row--selected');
        }
        ...
        const textEl = rowEl.createDiv({ cls: 'korian-opencode-model-picker-row-text' });

        const headerEl = textEl.createDiv({ cls: 'korian-opencode-model-picker-row-header' });
        headerEl.createEl('span', {
          cls: 'korian-opencode-model-picker-row-name',
          text: model.modelLabel,
        });
        const badgeEl = headerEl.createEl('span', {
          cls: 'korian-opencode-model-picker-row-badge',
          text: model.providerLabel,
        });
        if (!model.isAvailable) {
          badgeEl.classList.add('korian-opencode-model-picker-row-badge--unavailable');
          badgeEl.setText('Unavailable');
          badgeEl.title = 'Configured model not currently reported by OpenCode';
        }

        textEl.createDiv({
          cls: 'korian-opencode-model-picker-row-meta',
          text: model.rawId,
        });

        if (model.description) {
          textEl.createDiv({
            cls: 'korian-opencode-model-picker-row-desc',
    ...
    const commandsDesc = container.createDiv({ cls: 'korian-sp-settings-desc' });
    ...
    if (opencodeWorkspace?.agentStorage) {
      ...
      const subagentsDesc = container.createDiv({ cls: 'korian-sp-settings-desc' });
      ...
      const subagentsContainer = container.createDiv({ cls: 'korian-slash-commands-container' });
