...
const OPENCODE_AUX_AGENT_IDS: Record<OpencodeAuxAgentProfile, string> = {
  passive: 'korian-aux-passive',
  readonly: 'korian-aux-readonly',
};
...
export class OpencodeAuxQueryRunner implements AuxQueryRunner {
  ...
  constructor(
    private readonly plugin: KorianPlugin,
...
function buildOpencodeAuxAgentConfig(profile: OpencodeAuxAgentProfile): OpencodeManagedAgentConfig {
  ...
  if (profile === 'readonly') {
    return {
      definition: {
        description: 'Internal Korian read-only agent for OpenCode auxiliary tasks.',
        mode: 'primary',
        permission: {
          '*': 'deny',
          codesearch: 'allow',
  ...
  return {
    definition: {
      description: 'Internal Korian no-tool agent for OpenCode auxiliary tasks.',
      mode: 'primary',
      permission: {
        '*': 'deny',
        external_directory: 'deny',
