...
import { KorianSettingsStorage, type StoredKorianSettings } from '../settings/KorianSettingsStorage';
...
export class SharedStorageService implements SharedAppStorage {
  ...
  async initialize(): Promise<{ korian: Record<string, unknown> }> {
    await this.ensureDirectories();
    const korian = await this.korianSettings.load();
    return { korian };
  }

  async saveKorianSettings(settings: Record<string, unknown>): Promise<void> {
    await this.korianSettings.save(settings as StoredKorianSettings);
  }
