...
├── settings/       # base (shared .korian-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: Korian-owned classes should use the `.korian-` prefix; shared Obsidian host selectors and generic state classes may remain unprefixed
- **BEM-lite**: Prefer `.korian-{block}`, `.korian-{block}-{element}`, `.korian-{block}--{modifier}` for Korian-owned selectors
